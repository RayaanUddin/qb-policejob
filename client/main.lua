...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and (PlayerJob.name ~= "police" or PlayerJob.name ~= "bcso") then
        if DutyBlips then
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    if JobInfo.name ~= "police" or PlayerJob.name == 'bcso' then
        if DutyBlips then
            for _, v in pairs(DutyBlips) do
                RemoveBlip(v)
            end
...
RegisterNetEvent('police:client:UpdateBlips', function(players)
    if PlayerJob and (PlayerJob.name == 'police' or PlayerJob.name == 'bcso' or PlayerJob.name == 'ambulance') and
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    ...
    QBCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'bcso')
