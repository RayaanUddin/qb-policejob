...
local function IsArmoryWhitelist() -- being removed
    ...
    if (QBCore.Functions.GetPlayerData().job.name == 'police' or QBCore.Functions.GetPlayerData().job.name == 'bcso') then
        retval = true
    end
    return retval
end
...
function MenuGarage(currentSelection)
    ...
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.name == "bcso" then
            authorizedVehicles = Config.AuthorizedVehiclesbcso[QBCore.Functions.GetPlayerData().job.grade.level]
        end
    end)
...
RegisterNetEvent('qb-police:client:openArmoury', function()
    ...
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "bcso" then
            for _, armoryItem in pairs(Config.Itemsbcso.items) do
...
local function dutylistener()
    ...
    CreateThread(function()
        while dutylisten do
            if (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
...
local function stash()
    CreateThread(function()
        while true do
            Wait(0)
            if inStash and (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
...
local function trash()
    CreateThread(function()
        while true do
            Wait(0)
            if inTrash and (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerServerEvent("inventory:server:OpenInventory", "stash", "policetrash", {
                        maxweight = 4000000,
...
local function fingerprint()
    CreateThread(function()
        while true do
            Wait(0)
            if inFingerprint and (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
...
local function armoury()
    CreateThread(function()
        while true do
            Wait(0)
            if inAmoury and (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
...
local function heli()
    CreateThread(function()
        while true do
            Wait(0)
            if inHelicopter and (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
...
local function impound()
    CreateThread(function()
        while true do
            Wait(0)
            if inImpound and (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
local function garage()
    CreateThread(function()
        while true do
            Wait(0)
            if inGarage and (PlayerJob.name == "police" or PlayerJob.name == "bcso") then
                if PlayerJob.onduty then sleep = 5 end
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['qb-target']:AddBoxZone("BcsoStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "BcsoStash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['qb-target']:AddBoxZone("BcsoTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                name = "BcsoTrash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
        ...
        -- Fingerprint
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['qb-target']:AddBoxZone("BCSOFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                name = "BCSOFingerprint_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                maxZ = v.z + 1,
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['qb-target']:AddBoxZone("BcsoArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                name = "BcsoArmory_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
