...
local function UpdateBlips()
    ...
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "bcso" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
...
local function GetCurrentCops()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "bcso") and v.PlayerData.job.onduty then
            amount += 1
        end
    end
    return amount
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso") and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("impound", Lang:t("commands.impound"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso") or Player.PlayerData.job.name == "mechanic") and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso") or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "police" or biller.PlayerData.job.name == "bcso" then
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    if biller.PlayerData.job.name == "police" then 
                        ...
                        exports['qb-management']:AddMoney('bcso', amount)
                    end
                else
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.name ~= "bcso" or not Player.PlayerData.job.onduty then return end
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso") or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
...
QBCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(_, cb)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "police" or v.PlayerData.job.name == "bcso") and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if not Player or not CuffedPlayer or not Player.Functions.GetItemByName("handcuffs") then return end
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if ((Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "bcso") or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.name ~= "bcso" then return end
